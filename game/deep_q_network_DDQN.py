...
class BrainDQN:
    ...
    def __init__(self, actions):
        ...
        self.W_fc1, self.b_fc1, self.W_fc2, self.b_fc2, self.h_fc1 = self.createQNetwork()
        ...
        self.W_fc1T, self.b_fc1T, self.W_fc2T, self.b_fc2T, \
    ...
    def _setDirName(self):
        self.dir_name = "/dqn_Double/"

    def createQNetwork(self):
        s = tf.placeholder('float', [None, 80, 80, 4])  # 输入层 连续4帧80x80x4的游戏图像
        ...
        w_conv2_1 = self.get_weight([3, 3, 16, 32])  # 连续两个卷积加池化
        ...
        w_conv3_1 = self.get_weight([3, 3, 32, 64])   # 连续三个卷积加池化
        ...
        w_conv3_3 = self.get_weight([3, 3, 64, 64])
        b_conv3_3 = self.get_bias([64])
        h_conv3_3 = tf.nn.relu(self.conv2d(h_conv3_2, w_conv3_3, 1) + b_conv3_3)
        h_pool3 = self.max_pool_2x2(h_conv3_3)  # 10*10*64
        h_conv3_flat = tf.reshape(h_pool3, [-1, 6400])  # 1*6400 扁平化
        w_fc1 = self.get_weight([6400, 512])  # 两个全连接层
        b_fc1 = self.get_bias([512])
        h_fc1 = tf.nn.relu(tf.matmul(h_conv3_flat, w_fc1) + b_fc1)
        w_fc2 = self.get_weight([512, self.actions])
        b_fc2 = self.get_bias([self.actions])
        readout = tf.matmul(h_fc1, w_fc2) + b_fc2  # 第二层输出：结果out 前向传播

        return s, readout, w_conv1_1, b_conv1_1, w_conv2_1, b_conv2_1, w_conv2_2, b_conv2_2, w_conv3_1, b_conv3_1, w_conv3_2, b_conv3_2, w_fc1, b_fc1, w_fc2, b_fc2, h_fc1
    ...
    def trainQNetwork(self):
        ...
        QValue_batch_for_action = self.QValue.eval(feed_dict={self.stateInput: nextState_batch})
        max_act4next = np.argmax(QValue_batch_for_action, axis=1)
        selected_q_next = QValue_batch[range(len(max_act4next)), max_act4next]
        for i in range(0, BATCH_SIZE):
            terminal = minibatch[i][4]
            if terminal:
                y_batch.append(reward_batch[i])
            else:
                y_batch.append(reward_batch[i] + GAMMA * np.max(selected_q_next[i]))
    ...
    def record_by_pic(self):
        ...
        # plt.figure()
        writer = SummaryWriter('log/dqn_Dueling')
        # writer.add_figure(tag='activation_function', figure=fig)
        count = 0
        for i in scores:
            writer.add_scalar('Score', i, count)
            count = count + 1
            print("i: ", i)
            print("count: ", count)
